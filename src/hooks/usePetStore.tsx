...
const createInitialStore = (): PetStoreData => {
  const shouldResetAllData =
    typeof window !== 'undefined' &&
    new URL(window.location.href).searchParams.get('reset-data') === '1'

  if (shouldResetAllData) {
    clearLocalAppState()
  }

  const initialSnapshot = loadLocalAppState(toPersistedAppState(buildDefaultStore()))
  const normalizedInitialState = normalizePersistedState(initialSnapshot)
  const shouldZeroStatusForTest =
    typeof window !== 'undefined' &&
    new URL(window.location.href).searchParams.get('test-zero-status') === '1'
...
export const PetProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (!hasHydratedRemoteStore || didRunDailyRefreshRef.current) {
      return
    }
  ...
  }, [hasHydratedRemoteStore, showToast])
  ...
  }, [hasHydratedRemoteStore, store.meta.sleepEndAt, showToast])
